...
public class AddCommandParser implements Parser<AddCommand> {
    ...
    public AddCommand parse(String args) throws ParseException {
        ...
        Set<Tag> tagList = ParserUtil
                        .parseTags(argMultimap.getAllValues(PREFIX_TAG));
        PersonType role = ParserUtil.parsePersonType(
                        argMultimap.getValue(PREFIX_ROLE).get()); // default
        // role

        Person person = Person.newPerson(name, phone, email, address, tagList,
                        role);
    ...
    private static boolean arePrefixesPresent(ArgumentMultimap argumentMultimap,
                    Prefix... prefixes) {
        return Stream.of(prefixes).allMatch(prefix -> argumentMultimap
